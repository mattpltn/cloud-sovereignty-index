...
/** "DOC clause" for a source object, e.g. "EU-CSF §4 SOV-1". Empty string if absent. */
function sourceRef(source?: { doc?: string; clause?: string }): string {
  if (!source) return '';
  return [source.doc, source.clause].filter(Boolean).join(' ').trim();
}
...
function buildCsv(answers: AnswerMap, criteria: CriteriaFile, variant: 'EU-CSF' | 'Generalized'): string {
  const ctx = { variant };
  const rows: string[] = [
    ...
    '#',
    ['question_id', 'tier', 'objective_id', 'objective_title', 'question_title', 'question_text', 'source', 'answer', 'seal_contribution', 'points'].join(','),
  ];

  for (const obj of criteria.objectives) {
    for (const q of obj.questions) {
      const title = displayTitle(q, variant);
      if (q.type === 'single') {
        ...
        rows.push([
          ...
          `"${sourceRef(q.tiers.bloc.source).replace(/"/g, '""')}"`,
          blocAns?.value ?? '',
        ...
        if (q.tiers.national) {
          ...
          rows.push([
            ...
            `"${sourceRef(q.tiers.national.source).replace(/"/g, '""')}"`,
            natAns?.value ?? '',
...
export default function ExportAnswers({ assessmentId, criteria, result, variant }: Props) {
  ...
  function handleExport(format: ExportFormat) {
    ...
    if (format === 'json') {
      ...
      const csv = buildCsv(answers, criteria, variant);
